...
const execGit = (args: ReadonlyArray<string>, repoPath: string) =>
  Effect.tryPromise({
    try: () =>
...
const lines = (output: string): Array<string> =>
  output.trim().split("\n").filter(Boolean);
  ...
  "GitService",
  {
    effect: Effect.gen(function* () {
      ...
      const getRepositoryInfo = Effect.gen(function* () {
        ...
        return { name, branch, remote, path: repoPath };
      });
      ...
      const getCommitDiff = (shas: ReadonlyArray<string>) => {
        if (shas.length === 1) {
          return execGit(
      ...
      const stageFiles = (files: ReadonlyArray<string>) =>
      ...
      const unstageFiles = (files: ReadonlyArray<string>) =>
